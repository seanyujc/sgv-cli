...
export class Page extends Base implements IPage {
  ...
  constructor(private pageName: string) {
    ...
    this.routerTplPath = path.join(__dirname, "../../../", ".sgv/page/index.router.ts");
  }
    ...
    fs.readdir(this.templatePath, (err, files) => {
      ...
      files.forEach((fileName, index, array) => {
        ...
        fs.readFile(pathName, (error, data) => {
          const content = super.replaceKeyword(data.toString("utf8"), this.pageName);
  ...
  addFactoryFun(): void {
    const basePath = path.join(super.getCurrentDir(), "src/app/pages");
    const fileName = "factory.page.ts";
    const content = super.replaceKeyword(PAGE.FACTORY_FUNCTION_CONTENT, this.pageName);
    const addContent = PAGE.FACTORY_ANCHOR + super.endl() + content;

    super.addContentToFile(basePath, fileName, PAGE.FACTORY_ANCHOR, PAGE.FACTORY_ANCHOR, addContent, (err) => {
      if (err) {
        winston.error(err.message);
        return;
      }
  ...
  addRouter() {
    const basePath = path.join(super.getCurrentDir(), "src/app");
    const fileName = "index.router.ts";
    const original = fs.readFileSync(this.routerTplPath).toString("utf8");
    const configContent = PAGE.ROUTER_CONFIG_ANCHOR + super.endl() + "  "
      + super.replaceKeyword(PAGE.ROUTER_CONFIG_CONTENT, this.pageName);

    super.addContentToFile(basePath, fileName, original, PAGE.ROUTER_CONFIG_ANCHOR, configContent, (err) => {
      if (err) {
        winston.error(err.message);
        return;
      }
  ...
  deleteFactoryFun() {
    const basePath = path.join(super.getCurrentDir(), "src/app/pages");
    const fileName = "factory.page.ts";
    const pattern = super.replaceKeyword(PAGE.FACTORY_FUNCTION_PATTERN, this.pageName) + super.endl();
  ...
  deleteRouter() {
    const basePath = path.join(super.getCurrentDir(), "src/app");
    const fileName = "index.router.ts";
    const pattern = super.replaceKeyword(PAGE.ROUTER_CONFIG_PATTERN, this.pageName) + super.endl();
