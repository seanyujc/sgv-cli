...
export class Service extends Base implements IService {
    ...
    fs.readdir(this.templatePath, (err, files) => {
      ...
      files.forEach((fileName, index, array) => {
        ...
        fs.readFile(pathName, (error, data) => {
          ...
          const basePath = path.join(super.getCurrentDir(), "src/app/core/services");
  ...
  removeFiles() {
    const basePath = path.join(super.getCurrentDir(), "src/app/core/services", this.name + ".serv.ts");
    rimraf(basePath, (err) => {
      if (err) {
        winston.error(err.message);
        return;
  ...
  deleteFactoryFun() {
    const basePath = path.join(super.getCurrentDir(), "src/app/core");
    const fileName = "factory.serv.ts";
    let pattern = super.replaceKeyword(SERVICE.FACTORY_FUNCTION_PATTERN, this.name) + super.endl();
    ...
    super.deleteContentFromFile(basePath, fileName, pattern, (err) => {
      ...
      pattern = super.replaceKeyword(SERVICE.IMPORT_PATTERN, this.name) + super.endl();
      super.deleteContentFromFile(basePath, fileName, pattern, (err) => {
        if (err && err.name === "without") {
          winston.error("Without config option in factory file!");
          return;
  ...
  addServiceFun() {
    ...
    const interfaceContent = super.replaceKeyword(SERVICE.INTERFACE_CONTENT, this.funName);
    const addIFContent = SERVICE.INTERFACE_ANCHOR + super.endl() + interfaceContent;
    const funContent = super.replaceKeyword(SERVICE.FUNCTION_FUNCTION_CONTENT, this.funName);
    const addFunContent = SERVICE.FUNCTION_ANCHOR + super.endl() + funContent;

    super.addContentToFile(basePath, fileName, "", SERVICE.INTERFACE_ANCHOR, addIFContent, (err) => {
      if (err) {
        winston.error(err.message);
        return;
      }
      winston.info("Added " + this.funName + " interface in " + this.name + " service!");
      super.addContentToFile(basePath, fileName, "", SERVICE.FUNCTION_ANCHOR, addFunContent, (err) => {
        if (err) {
          winston.error(err.message);
          return;
        }
        winston.info("Added " + this.funName + " function in " + this.name + " service!");
      });
    });
  }
