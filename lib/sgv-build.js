...
if (options.service && !options.function) {
    const keyword = options.service;
    const serviceName = keyword.substring(keyword.lastIndexOf("/") + 1);
    const parentPath = keyword.substring(0, keyword.lastIndexOf("/"));
    service_1.buildService(parentPath, serviceName);
}
if (options.service && options.function) {
    const apiModules = service_1.getCurrentAPIModules();
    inquirer_1.default
        .prompt([
        {
            type: "list",
        ...
        const hosts = service_1.getHostListInAPIModule(apiModule);
        if (hosts.length > 1) {
            inquirer_1.default.prompt([]);
        }
        service_1.addFunctionInService(options.function, options.service, method, apiModule);
    });
}
